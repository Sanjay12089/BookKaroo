...
export function SearchDropdown({
        ...
        {recentSearches.length > 0 ? (
          <>
            <div className="flex items-center justify-between px-4 py-2">
              <span className="text-[11px] uppercase tracking-wider text-tx-muted font-sans font-semibold">
                Recent Searches
              </span>
              <button
                onClick={onClearRecent}
                className="text-[11px] text-brand hover:text-brand/80 font-sans"
              >
                Clear
              </button>
            </div>
            {recentSearches.map((q) => (
              <button
                ...
                className="flex items-center gap-3 w-full px-4 py-2.5 hover:bg-section text-left transition-colors"
              >
                <Clock size={14} className="text-tx-muted flex-shrink-0" />
                <span className="text-sm text-tx-secondary font-sans">{q}</span>
              </button>
            ))}
          </>
        ) : (
  ...
  if (query.length < 2) {
    return (
      <div className="px-4 py-6 text-center">
        <p className="text-sm text-tx-muted font-sans">Type at least 2 characters to search</p>
      </div>
    );
  }
  ...
  if (!data || data.totalResults === 0) {
    return (
      <div className="px-4 py-8 text-center">
        ...
        <p className="text-sm font-semibold text-tx-secondary font-sans">
          No results for &ldquo;{query}&rdquo;
        </p>
        <p className="text-xs text-tx-muted font-sans mt-1">
  ...
  return (
    <div className="py-2">
      {data.movies.length > 0 && (
        <section>
          <p className="px-4 py-2 text-[11px] uppercase tracking-wider text-tx-muted font-sans font-semibold">
            Movies
          </p>
          {data.movies.map((m) => (
            <button
              ...
              className="flex items-center gap-3 w-full px-4 py-2.5 hover:bg-section text-left transition-colors"
            >
              <div className="w-10 h-[60px] rounded overflow-hidden bg-section flex-shrink-0">
                {m.posterUrl ? (
                  <img
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-tx-primary font-display line-clamp-1">
                  {m.title}
                </p>
                  ...
                  <span
                    className={`text-[10px] font-sans font-semibold px-1.5 py-0.5 rounded ${
                      m.category === 'NowShowing'
                        ? 'bg-brand/12 text-[#FF6770]'
                        : 'bg-section text-tx-muted'
                    }`}
                  >
        ...
        <section>
          ...
          {data.events.map((e) => (
            <button
              ...
              className="flex items-center gap-3 w-full px-4 py-2.5 hover:bg-section text-left transition-colors"
            >
              <div className="w-10 h-10 rounded overflow-hidden bg-section flex-shrink-0">
                {e.posterUrl ? (
                  <img
        ...
        <section>
          ...
          {data.venues.map((v) => (
            <button
              ...
              className="flex items-center gap-3 w-full px-4 py-2.5 hover:bg-section text-left transition-colors"
            >
              <div className="w-10 h-10 rounded-full bg-section flex items-center justify-center flex-shrink-0">
                <Building2 size={18} className="text-tx-muted" />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-tx-primary font-sans line-clamp-1">
                  {v.name}
                </p>
        ...
        <section>
          ...
          {data.cities.map((c) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-tx-primary font-sans">
                  {c.name}
                </p>
                <p className="text-xs text-tx-muted font-sans">{c.state}</p>
              </div>
            </button>
          ))}
        </section>
      ...
      {data.totalResults > 8 && (
        <div className="border-t border-border-l mt-1">
          <button
            onClick={() => {
              onSelectResult(query);
              onClose();
            ...
            className="flex items-center justify-center w-full px-4 py-3 text-sm text-brand hover:bg-section font-sans transition-colors"
          >
