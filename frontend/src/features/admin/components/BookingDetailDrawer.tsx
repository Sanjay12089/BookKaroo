    ...
    <AnimatePresence>
      {bookingRef && (
        <>
          ...
          >
            {/* Header */}
            <div className="sticky top-0 bg-card border-b border-border-l px-6 py-4 flex items-center gap-3">
              <div>
                <p className="font-bold text-base text-tx-primary font-sans">Booking Detail</p>
                {booking && (
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className="font-mono text-[11px] bg-section px-2 py-0.5 rounded text-tx-muted">
                      {booking.bookingRef}
                    </span>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4 space-y-5">
              {isLoading ? (
                ...
                <p className="text-tx-muted text-sm text-center py-12">Booking not found.</p>
              ) : (
                <>
                  {/* Movie / Show */}
                  <section>
                    <p className="text-[11px] text-tx-muted uppercase tracking-wider font-sans mb-2">Movie / Show</p>
                    <div className="flex gap-3">
                      {posterSrc
                        ? <img src={posterSrc} alt="" className="w-[56px] h-[84px] object-cover rounded-lg flex-shrink-0" />
                        : <div className="w-[56px] h-[84px] rounded-lg bg-section flex-shrink-0" />}
                      <div className="min-w-0">
                        <p className="font-bold text-base text-tx-primary font-display leading-snug">
                          {booking.movieTitle ?? booking.eventTitle ?? '—'}
                        </p>
                        {(booking.format || booking.language) && (
                          <p className="text-[12px] text-tx-muted mt-0.5 font-sans">
                            {[booking.format, booking.language].filter(Boolean).join(' · ')}
                          </p>
                        )}
                        <p className="text-[12px] text-tx-secondary mt-1 font-sans">📅 {booking.showDateLabel}</p>
                        <p className="text-[12px] text-tx-secondary font-sans">🕐 {booking.showTimeLabel}</p>
                        <p className="text-[12px] text-tx-muted font-sans">📍 {booking.venueName}</p>
                        <p className="text-[11px] text-tx-muted font-sans">{booking.screenName} · {booking.cityName}</p>
                      </div>
                    </div>
                  </section>
                  ...
                  {/* Customer */}
                  <section>
                    <p className="text-[11px] text-tx-muted uppercase tracking-wider font-sans mb-2">Customer</p>
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <p className="font-bold text-[14px] text-tx-primary">{booking.userName}</p>
                        <p className="text-[12px] text-tx-muted truncate">{booking.userEmail}</p>
                        <p className="text-[12px] text-tx-muted font-mono">{booking.userMobile}</p>
                      </div>
                    </div>
                  </section>
                  ...
                  {/* Seats */}
                  <section>
                    <p className="text-[11px] text-tx-muted uppercase tracking-wider font-sans mb-2">Seats</p>
                    {booking.seatsSummary ? (
                      <div className="flex flex-wrap gap-1.5 mb-2">
                        {booking.seatsSummary.split(', ').map((s) => (
                          <span key={s} className="font-mono text-[11px] bg-section px-2 py-1 rounded text-tx-primary">
                            {s}
                          </span>
                        ))}
                      </div>
                    ) : <p className="text-tx-muted text-sm">No seat data</p>}
                    <p className="text-[12px] text-tx-muted font-sans">
                  ...
                  {/* Pricing Breakdown */}
                  <section>
                    <p className="text-[11px] text-tx-muted uppercase tracking-wider font-sans mb-2">Pricing Breakdown</p>
                    <div className="space-y-1.5 text-[13px] font-sans">
                      ...
                      <hr className="border-border-l my-1" />
                      <div className="flex justify-between font-bold text-[16px]">
                        <span className="text-tx-primary">Amount Paid</span>
                        <span className="text-brand">{fmt(booking.amountPaid)}</span>
                      </div>
                    </div>
                  </section>
                  ...
                  {/* Payment */}
                  <section>
                    <p className="text-[11px] text-tx-muted uppercase tracking-wider font-sans mb-2">Payment</p>
                    <div className="space-y-1 text-[13px] font-sans">
                      <Row label="Method" value={booking.paymentMethod ?? '—'} />
                      <div className="flex justify-between">
                        <span className="text-tx-muted">Transaction ID</span>
                        <span className="font-mono text-[11px] text-tx-secondary">{booking.providerPaymentId ?? '—'}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-tx-muted">Status</span>
                        <span className={`text-[11px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider ${STATUS_CLASSES[booking.paymentStatus] ?? 'bg-section text-tx-muted'}`}>
                      ...
                      {booking.refundAmount != null && (
                        <>
                          ...
                          <div className="flex justify-between">
                            <span className="text-tx-muted">Refund ID</span>
                            <span className="font-mono text-[11px] text-tx-secondary">{booking.refundId}</span>
                          </div>
                        </>
                      )}
                    </div>
                  ...
                  {/* Documents */}
                  <section>
                    <p className="text-[11px] text-tx-muted uppercase tracking-wider font-sans mb-2">Documents</p>
                    <div className="flex gap-2">
                      <button
                        disabled={!booking.invoiceUrl}
                        onClick={() => booking.invoiceUrl && window.open(booking.invoiceUrl, '_blank')}
                        className="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-border-l text-[12px] text-tx-secondary hover:text-tx-primary disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                      >
                  ...
                  <>
                    ...
                    {(booking.status === 'Confirmed' || booking.status === 'Cancelled') && booking.refundAmount == null && (
                      <button
                        ...
                        className="w-full py-2.5 rounded-lg border border-warning text-warning text-[13px] font-semibold hover:bg-warning-bg/10 transition-colors"
                      >
                        Process Refund
                      </button>
                    )}
                    <button
                      ...
                      className="w-full py-2.5 rounded-lg border border-border-l text-tx-secondary text-[13px] hover:bg-section transition-colors disabled:opacity-60"
                    >
