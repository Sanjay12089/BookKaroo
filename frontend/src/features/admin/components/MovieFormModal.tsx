          ...
          <button
            ...
            className={cn(
              ...
              selected
                ...
                : 'border-border-l text-tx-muted hover:border-border-m hover:text-tx-secondary',
            )}
          >
            <span className={cn(
              'w-3.5 h-3.5 rounded-sm border flex items-center justify-center flex-shrink-0 transition-colors',
              selected ? 'bg-brand border-brand' : 'border-border-m',
            )}>
        ...
        return (
          ...
          >
            <span className={cn(
              ...
              selected ? 'border-brand' : 'border-border-m',
            )}>
  ...
  return (
    <Modal open onClose={onClose} maxWidth="max-w-3xl"
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-0">
        ...
        {mode === 'create' && (
          <div className="rounded-xl border border-brand/30 bg-brand/06 p-4 mb-5">
            <p className="text-[13px] font-semibold text-tx-primary mb-2">Import from TMDB</p>
            <div className="flex gap-2">
              ...
              <button type="button" onClick={handleTmdbSync} disabled={syncMutation.isPending}
                className="px-4 py-2 rounded-lg bg-brand text-white text-sm font-semibold font-sans disabled:opacity-60">
                {syncMutation.isPending ? 'Syncing…' : 'Sync'}
              </button>
            </div>
            <button type="button" onClick={() => importMutation.mutate()} disabled={importMutation.isPending}
              className="mt-2 text-xs text-brand font-sans hover:underline disabled:opacity-60">
