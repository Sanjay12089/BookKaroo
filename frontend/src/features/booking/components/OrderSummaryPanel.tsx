  ...
  return (
    <div className="sticky top-24 space-y-4">
      <div className="p-5 rounded-xl bg-card border border-border-l">
        <p className="text-[11px] font-semibold text-tx-muted uppercase tracking-widest mb-4 font-sans">
          Order Summary
        </p>
        ...
        <div className="space-y-3 text-sm font-sans">
          ...
          {breakdown.discount > 0 && (
            <div className="flex justify-between text-success font-semibold text-[11px] uppercase tracking-wider">
        ...
        <div className="my-4 border-t-2 border-border-l" />

        <div className="flex justify-between items-baseline">
          <span className="text-sm font-semibold text-tx-secondary font-sans">Amount Payable</span>
          <span className="font-display font-black text-2xl text-brand">₹{breakdown.amountPaid}</span>
        </div>

        {/* Coupon section */}
        <div className="mt-5">
          {coupon ? (
            <div className="flex items-center gap-2 p-3 rounded-lg bg-success-bg/10 border border-success-border/30">
              <span className="text-success text-sm font-sans flex-1">
                ✓ {coupon.code} applied — ₹{coupon.discount} off!
              </span>
              <button
                onClick={onRemoveCoupon}
                className="px-2.5 py-1 rounded-full text-xs font-sans border border-border-l text-tx-muted hover:border-error hover:text-error transition-colors"
              >
                ✕ Remove
              </button>
            </div>
            ...
            <div className="flex gap-2">
              <input
                ...
                className={cn(
                  'flex-1 px-3 py-2 rounded-lg bg-page border text-sm font-mono text-tx-primary',
                  'placeholder:text-tx-muted focus:outline-none focus:ring-1 focus:ring-accent-indigo',
                  couponError ? 'border-error' : 'border-border-l'
                )}
              />
              <button
                onClick={onApplyCoupon}
                disabled={!couponInput || couponLoading}
                className="px-4 py-2 rounded-lg bg-brand text-white text-sm font-semibold font-sans disabled:opacity-50 hover:opacity-90 transition-opacity"
              >
                {couponLoading ? '…' : 'Apply'}
              </button>
            </div>
          ...
          {couponError && (
            <p className="mt-1 text-xs text-error font-sans">{couponError}</p>
          )}
          {isDev && !coupon && (
            <p className="mt-2 text-[10px] text-tx-muted font-sans">
        ...
        <label className="flex items-start gap-2 mt-4 cursor-pointer">
          ...
          />
          <span className="text-xs text-tx-muted font-sans leading-relaxed">
            I agree to the{' '}
            <span className="text-brand">Terms &amp; Conditions</span> and{' '}
            <span className="text-brand">Cancellation Policy</span>.
        ...
        <div className="mt-3 text-center">
          <p className="text-xs text-tx-muted font-sans">Visa · Mastercard · RuPay · UPI · NetBanking</p>
        </div>
      </div>
    </div>
  );
