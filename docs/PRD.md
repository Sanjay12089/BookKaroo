...
**PR flow:** branch → commit → push → PR to `develop` → tests pass → merge → periodic release to `main`

**Handoff:** When Claude Code session reaches ~80% context, generate `/docs/HANDOFF.md` with current feature status, open bugs, next priorities, and setup instructions.
