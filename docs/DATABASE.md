...
> ⚠️ **Source-of-truth warning:** this repo currently has **three separate schema definitions** that have drifted out of sync:
> 1. `bookkaroo-db/migrations/001_init.sql` — hand-written, `snake_case` tables (`users`, `bookings`, ...). What this doc describes below.
> 2. `backend/database/migrations/*.sql` (14 files) — a second, separate hand-written migration set.
> 3. **The actual live schema** — generated by EF Core (`backend/src/BookKaroo.Infrastructure/Data/Migrations/`), using **`PascalCase` tables** (`"Users"`, `"Bookings"`, ...). This is what the running API actually reads and writes (see `backend/CLAUDE.md`). A few newer tables (`LysOrganizers`, `LysEvents`, `LysUploads`) are created via raw `CREATE TABLE IF NOT EXISTS` SQL executed directly in `Program.cs` at startup, bypassing EF migrations entirely.
>
> **If you're setting up a fresh environment, apply the EF Core migrations** (`dotnet ef database update`, see root `README.md`), not the SQL files below — they will not produce a schema the app can actually use. This doc is kept as a readable reference for the column-level GST/pricing additions, not as an authoritative source. Reconciling the three schema sources into one is an open item — see the note at the bottom of this file.
...
Not documented here at all: `PartnerProfile`, `PartnerVenueAccess`, `LysOrganizer`, `LysEvent`, `LysUpload`, `EventTicketLock` — all real, live entities (see `backend/src/BookKaroo.Domain/Entities/` and `backend/database/migrations/013_PartnerPortal.sql` / `014_AddLysFeature.sql`) added after this doc was last updated. Whoever picks this up next should pick **one** schema source (almost certainly the EF Core migrations, since that's what the app actually runs against), delete or archive the other two, and regenerate this doc from the real `BookKarooDbContext` model.
